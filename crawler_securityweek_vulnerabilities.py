# -*- coding: utf-8 -*-
"""
Created on Thu Jun 14 11:09:37 2018
...
#        self.realtimeNewsURL=[]
    ...
    def ConnDB(self):
        ...
        #if 'https://www.securityweek.com/rarstone-rat-being-used-targeted-attacks-asia-trend-micro' in AddressLst:
         #   print 1
    ...
    def CrawHistoryNews(self,startPage,endPage,url_Part_1,result):
        ...
        for pageID in range(startPage,endPage+1):
            print pageID
            if pageID==1:
                urls.append('https://www.securityweek.com/virus-threats/vulnerabilities')
            else:
                ...
                if 'class' in bs_object.attrs and bs_object.get('class')==['panel-pane','pane-block','pane-views-news-industry-block-1']:
                    ...
                    for a in a_list:
                        if a.parent.parent.span and 'class'in a.parent.parent.span.attrs and a.parent.parent.span.get('class')== ['field-content'] :
                            #print a.attrs
                            AddressLst=self.extractData(['Address'])
                            if 'https://www.securityweek.com'+a.get('href') not in AddressLst:
                                print 8
...
#        print(' Using ' + str(self.ThreadsNum) + ' threads for collecting news ... ')
    ...
    def coroutine_run(self,totalPages,Range,initPageID,**kwarg):
        ...
        gevent.joinall(jobs)
#result=[]
#k= WebCrawlFromthreatpost(IP= '10.108.217.52',PORT=27017,dbName="test",collectionName="zhou-vulnerabilities-securityweek",ThreadsNum=6).multi_threads_run(305,10,1,url_Part_1='https://www.securityweek.com/virus-threats/vulnerabilities?page=') 
j= WebCrawlFromthreatpost(IP= '10.108.217.52',PORT=27017,dbName="test",collectionName="zhou-vulnerabilities-securityweek",ThreadsNum=6).ConnDB()
